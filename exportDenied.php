...
$query = $conn->prepare("SELECT *, MONTHNAME(date_added) as months FROM requests 
WHERE form_status = ? GROUP BY months ASC"); 
 $query->execute(["denied"]);
if($query->rowCount() > 0){
    $delimiter = ","; 
    $filename = "denied-data_" . date('Y-m-d') . ".csv"; 
